...
- Two different ways of mapping images to channel names are demonstrated here. There are several others, and often you could create several correct mappings for a given set of images, but these may serve as a helpful example to refer to in your own work.
  ...
  - The `.npy` illumination correction functions are assigned a name based on a unique string in the name (such as `IllumER`). As there is only one set of illumination correction functions for each entire plate, the image sets cannot simply be constructed by using `Image set matching` as `Order`.
...
Since microscope objectives don't typically have a completely uniform illuminatio pattern, applying an illumination correction function can help make segmentation better and measurements more even by compensating for this.  Pay close attention to the top of the field of view to see the greatest effect.
...
- Examine the segmentation and adjust the segmentation parameters until you feel you're ready to test them on another image; they need not be
...
- **After** the `IdentifyPrimaryObjects` module for Nucleoli but **before** the `RelateObjects` modules, add a MaskImage module (from
